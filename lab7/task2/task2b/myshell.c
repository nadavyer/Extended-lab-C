...
void execute_piped_commands(cmdLine *command);

int execute(cmdLine *command);
...
void execute_piped_commands(cmdLine *command){
    int pipes_counter = 0;
    int command_num = 0;
    cmdLine* curr = command;
    while(curr != NULL){
        pipes_counter++;
        curr = curr->next;
    }
    int** pipes = createPipes(pipes_counter-1);
    int pid[pipes_counter];
    curr = command;
    while(curr != NULL){
        pid_t cpid1;
        cpid1 = fork();
        ...
        int * rpipe = rightPipe(pipes, curr);
        int * lpipe = leftPipe(pipes, curr);
        ...
        if(cpid1 == 0) {
            ...
            if (rpipe != NULL) {
                ...
                dup(rpipe[1]);
                close(rpipe[1]);
                fprintf(stderr,"rpipe:  %s\n", curr->arguments[0]);
            }
            int exit_val = execute(curr);
            exit(exit_val);
        } else {            /*done by the parent*/
            if (rpipe != NULL){
                fprintf(stderr,"parent close rpipe: %s\n", curr->arguments[0]);
                close(rpipe[1]);
            }
            if (lpipe != NULL){
                fprintf(stderr,"parent close lpipe: %s\n", curr->arguments[0]);
                close(lpipe[0]);
            }
    ...
    releasePipes(pipes, pipes_counter-1);
    fprintf(stderr, "parent released the pipes and finished successfully\n");
}
