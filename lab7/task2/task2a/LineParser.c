...
cmdLine *copy_cmd_line(cmdLine *cmd_line)
{
    cmdLine *new_line = (cmdLine *)malloc(sizeof(cmdLine));
    new_line->argCount = cmd_line->argCount;
    new_line->inputRedirect = NULL;
    new_line->outputRedirect = NULL;
    new_line->blocking = cmd_line->blocking;
    new_line->idx = 0;
    new_line->next = NULL;
    ...
    {
        ((char **)new_line->arguments)[i] = strClone(cmd_line->arguments[i]);
    }
