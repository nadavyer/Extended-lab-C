...
int main(int argc, char **argv) {
    ...
    else if(iFlag == 1 && rFlag == 1) {
        char *format = "byte %d %02X %02X\n";
        char *line = NULL;
        size_t len = 0;
        unsigned char orgFileChar = '\0';
        unsigned char newFileChar = '\0';
        int offset = 0;
        while (getline(&line, &len, readFromFile) != -1) {
            sscanf(line, format, &offset, &orgFileChar, &newFileChar);
            struct diff *diffToInsert = malloc(sizeof(diff));
            diffToInsert->offset = offset;
            diffToInsert->orig_value = orgFileChar;
            diffToInsert->new_value = newFileChar;
    ...
    if (rFlag == 1) {
        ...
        if (n == -1) {
            while (curr != NULL) {
                fseek(newFile, curr->diff_data->offset, SEEK_SET);
                unsigned char byteToModify = curr->diff_data->orig_value;
                fwrite( &byteToModify, sizeof(byteToModify), 1, newFile);
                curr = curr->next;
            }
        } else{
            int k;
            for (k = 0; k < n && curr !=NULL; k++){
                fseek(newFile, curr->diff_data->offset, SEEK_SET);
                unsigned char byteToModify = curr->diff_data->orig_value;
                fwrite( &byteToModify, sizeof(byteToModify), 1, newFile);
                curr = curr->next;
