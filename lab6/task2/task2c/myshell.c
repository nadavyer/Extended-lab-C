...
void handleHistory(cmdLine *cmdLine, cmdHistoryNode *head) {
    if ((cmdLine->argCount > 2) && (strcmp("history", cmdLine->arguments[0]) == 0) &&
        (strcmp("-d", cmdLine->arguments[1]) == 0)) {
        long int d = strtol(cmdLine->arguments[2], NULL, 10);
        if(d >= SIZE_OF_LIST) {
            fprintf(stdout,"%s\n", "Index out of history bounderis");
            return;
        }
...
void handleSimanKriah(cmdLine **cmdLine, cmdHistoryNode *head) {
    if (strncmp((*cmdLine)->arguments[0], "!", 1) != 0)
        return;

    char buf[10];
    strcpy(buf, (*cmdLine)->arguments[0] + 1);
    long int cmd_num = strtol(buf, NULL, 10);

    long int iter = (SIZE_OF_LIST-cmd_num)-1;
    cmdHistoryNode *hist_cmd = head;
    while (iter--) {
        hist_cmd = hist_cmd->next;
    }
    *cmdLine = copy_cmd_line(hist_cmd->cmd);
